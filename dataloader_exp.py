...
class Myarticles(data.Dataset):
    def __init__(self, csvfile_path, txt_folder_path, glove_path='/Users/duanyiqun/Downloads/Textcls/glove.6B'):
...
def Creat_article_list(csvfile, label= 'Basic '):
    df = pd.read_csv(csvfile)
    article_list = []
    for idx, cont in enumerate(df['Basics ']):
        if cont != 'NA':
            article_list.append([df['Name'][idx],cont])
    return article_list
at = Creat_article_list('/Users/duanyiqun/Downloads/Textcls/wp_analysis.csv')
at = at[:500]
...
print(at[0][0])
filepath = os.path.join('./txt',at[1][0])

with open(filepath) as f:
    article = f.read()
    article = re.sub(r"\n", "", article)
    #article = re.sub("1234567890", "",article)
    article = SenToken(article)
    print(article)
...
#article=steamize_words(article)
#print(article)
...
def steamize_words(article):
    snowball_stemmer = SnowballStemmer('english')
    for idx, sentence in enumerate(article):
        for idx, word in enumerate(sentence):
            sentence[idx] = snowball_stemmer.stem(word)
        article[idx] = sentence
        print(article[idx])
    return article

words_tokenized=steamize_words(words_tokenized)
snowball_stemmer = SnowballStemmer('english')

#print(snowball_stemmer.stem('Marketing'))
